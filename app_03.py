...
def write_txt(temperature, context_length, state, chat_type):
    today = date.today()
    file_name = "chat_" + today.strftime("%b-%m-%Y") + ".txt"
    start = int(chat_type)
    ...
    with gr.Column(elem_id="col-container"):
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                with gr.Blocks():
                    with gr.Row():
                        chat_type = gr.Dropdown(label="Set a custom insruction for the chatbot:", choices=["All messages", "Response only"], type="index")
